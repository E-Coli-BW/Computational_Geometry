...
public class Polygon {
  ...
  public Polygon copy(){
    ...
    for(Vert v:this.verts){
      Vert vcopy = out.new Vert(v.p,v.incoming,v.outgoing);
      out.verts.add(vcopy);
      vcopy.informEdges();
    }
    for(Edge e:this.edges){
      Edge ecopy = out.new Edge(e);
      out.edges.add(ecopy);
      ecopy.informVerts();
    }
    for(Vert v:this.verts){
  ...
  public void draw (Graphics2D g) {
    // CW inout=2, draw it blue
    // CCW inout=1, draw it red
    if(this.getInOut()==2){
      for (Edge e : edges){
        e.draw(g,Color.blue);
      }
  ...
  public Polygon intersection (Polygon that){
    ...
    //turn this and that back to original
    this.invert();
    that.invert();
    return out;
  ...
  // Union is still buggy.... didn't realize it until I implemented 
