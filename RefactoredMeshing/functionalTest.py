...
path=r"C:\\Spring 2019 UM Class\\CSC 647 Computational Geometry\\FinalProject\\RefactoredMeshing\\m0.txt"
reader=FileReader()
data=reader.readTxt(path)
points=reader.constructPoints(data)

# for i in range(len(points)):
# 	for nn in range(len(points[i].nnlist)):
# 		if(points[i].nnlist[nn] in points):
# 			nn_index=points.index(points[i].nnlist[nn])
# 			# update info about nnlist[nn]
# 			points[i].nnlist[nn].nnlist=points[nn_index].nnlist

# points_with_id=[(i,points[i]) for i in range(len(points))]
...
for i in range(len(points)):
	...
	# print("nnlist:",points[i].nnlist)

	# print(point.nnlist[1])
	# print("nnlist length:",len(point.nnlist))
#----------------------------------------------------BUG----------------------------
	# well... here's a huge bug... when you add points to nnlist
	# those nnlist's points don't know their neighbors yet....
	# so every end point in each Edge does not know its neighbor...
	# don't yet know how to resolve this problem....
	# Maybe I can add an update function in Point class
	# 
#----------------------------------------------------BUG----------------------------
...
print(len(FinalRes))
writer=FileWriter()
path=r"C:\\Spring 2019 UM Class\\CSC 647 Computational Geometry\\FinalProject\\RefactoredMeshing\\"
